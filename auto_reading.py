...
    stop_words = [line.strip('\n') for line in open('stop_words.txt'	 encoding='utf-8').readlines()]  	
...
        self.text = re.sub('[^\u4e00-\u9fa5。，！：、]{3	}'	 ''
        self.text = re.sub('{IMG:.?.?.?}'	 ''	 self.text)  
        self.text = re.sub('<!--IMG_\d+-->'	 ''	 self.text)  
        self.text = re.sub('(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:	.;]+[-A-Za-z0-9+&@#/%=~_|]'	 ''
        self.text = re.sub('<a[^>]*>'	 ''	 self.text).replace("</a>"
        self.text = re.sub('<P[^>]*>'	 ''	 self.text).replace("</P>"
        self.text = re.sub('<strong[^>]*>'	 '	'
        self.text = re.sub('<br>'	 '	'
        self.text = re.sub('www.[-A-Za-z0-9+&@#/%?=~_|!:	.;]+[-A-Za-z0-9+&@#/%=~_|]'	 ''
...
    result_df.columns = ['标题'	'作者'	'人物'
    return result_df  		
  		
  		
if __name__ == "__main__":  		
    #内容提取和词云  		
    # readingNews1 = ReadingNews('https://mp.weixin.qq.com/s/2ZgvR_FMmcrYYNJkXAqyzQ'	 channel='weixin')  	
    readingNews1 = ReadingNews('https://mp.weixin.qq.com/s?src=11×tamp=1614583801&ver=2919&signature=cC4IPeZKUH6-wNtRBOa72nzlVIFcb6cR7CzHbWTQ35dsHVUnQeuIcq4ylYwAHxU-czYBP2jUcz2GD1kpq9MkUJV-tPlGReJHmZlGGXdPX3RHYwXi9uEQWcdIlDT5w9OT&new=1'	 channel='weixin')  	
